...
MODES = {
    # ✅ 10 gems per cycle (2-step: unclaim + claim) — from Currency-Adder-v5
    "gems10": {
        "label":"💎 10 Gems", "unit":"Gems", "type":"reward_gems10",
        "templateId":128800,
...
# ── 10 GEMS UNCLAIM MUTATION (step 1 of 2) — from Currency-Adder-v5 ──
...
# ── 10 GEMS CLAIM MUTATION (step 2 of 2) — from Currency-Adder-v5 ──
...
def do_single(hdr, mode_key):
    ...
    try:
        if m["type"] == "reward_gems10":
            # 2-step: unclaim then claim (from Currency-Adder-v5)
            ...
            if not (attrs and attrs[0].get("attributeValue") == "2"):
                return False
            ...
            return bool(rewards and rewards[0].get("responseStatus") is True)
...
def start():
    ...
    if mode_key not in MODES or mode_key == "elite_internal": mode_key = "gems10"
...
<head>
<meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>GALAXY FARM · DC25 v6.3 UPDATED</title>
...
<div class="hdr">
  ...
  <div class="new-method">✨ v6.3: 10 GEMS · 4 GEMS · 30 TICKETS · 2 ELITE</div>
...
const CIRC=245;
const MCFG={
  gems10:   {lbl:'💎 DESIRED GEMS (10/click)',hint:'2-step: unclaim+claim = 10 gems ⚡', hintNew:true, btn:'LAUNCH', cls:'',        chain:false,icon:'💎',epc:0},
  gems:     {lbl:'💎 DESIRED GEMS (4/click)', hint:'⚡ 1 click = 4 gems (NEW!)', hintNew:true, btn:'LAUNCH', cls:'',        chain:false,icon:'💎',epc:0},
...
const COLS={gems10:'var(--cgems)',gems:'var(--cgems)',tickets:'var(--ctickets)',coins:'var(--ccoins)',elite:'var(--celite)',legendary:'var(--clegend)',champion:'var(--cchamp)'};
const ICONS={gems10:'💎',gems:'💎',tickets:'🎫',coins:'🪙',elite:'🃏',legendary:'⭐',champion:'👑'};
  const MK=['gems10','gems','tickets','coins','elite','legendary','champion'];
  const MI=['💎','💎','🎫','🪙','🃏','⭐','👑'];
  const ML=['GEMS×10','GEMS×4','TICKETS','COINS','ELITE','LEGEND','CHAMP'];
  const MS=['10/click⚡','4/click⚡','+30/click','+100/click','+2/click','AUTO⚡','AUTO⚡'];
